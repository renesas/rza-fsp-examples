...
fsp_err_t uart_ep_demo(void)
{
    ...
    err = uart_print_user_msg((uint8_t *)"Open Serial Terminal with this baud rate value and provide input\r\n");
    err = uart_print_user_msg((uint8_t *)"ranging from 1 - 2000 to set time cycle values in milliseconds\r\n");
    ...
    while (true)
    {
        if(g_data_received_flag)
        {
            ...
            if(b_valid_data)
            {
                err = uart_print_user_msg((uint8_t *)"Input value: ");
